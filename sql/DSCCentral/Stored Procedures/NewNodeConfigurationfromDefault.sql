...
BEGIN
	...
	BEGIN TRY
		IF @NodeConfigurationName IS NULL
		BEGIN
			/*We Want the default which is the first record in the table added from setup*/
			SELECT @NodeConfigurationName = [NodeConfigurationName]
			FROM [dbo].[NodeConfigurationDefault] where DefaultID =1
		END
		...
		IF @Payload IS NULL
		BEGIN
			SELECT @DefaultNodePayload = payload
			FROM NodeConfigurationDefault
			WHERE NodeConfigurationName = @NodeConfigurationName
		END
