...
"@
    ...
    if (Test-Path -Path $reqPath)
    {
        ...
        {
            $submitRequest = & $ENV:SystemRoot\system32\certreq.exe @('-submit','-q','-config',$CA,$ReqPath,$CerPath)
        } # if
    ...
    if (Test-Path -Path $cerPath)
    {
        ...
        $acceptRequest = & $ENV:SystemRoot\system32\certreq.exe @('-accept','-machine','-q',$cerPath)
